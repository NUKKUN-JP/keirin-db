...
v7からの変更点 (グレード・日目・級班):
  winticket API の返り値を実測し、次が取れることを確認した。
    cups[].grade      グレード(数値)。DBの文字と152件突き合わせて確定:
                        1 -> F2 (97件) / 2 -> F1 (34件) / 3 -> G3 (21件)
                      4以上(G2/G1/GP)は今回のデータに現れなかったので、
                      未知の数値は空欄にしてログに出す。
    schedule.day      何日目か
    cups[].duration   開催日数。day と合わせて「最終日」を判定する
    entries[].playerCurrentTermClass / Group
                      選手の級班。class 1=S級 2=A級 / group が班。
                      A3の例(class=2,group=3)から。S級の例も見て確かめる。
  S/B取得と同じAPIなので、追加の通信は要らない。
...
def main():
    ...
    if os.path.exists(out_path):
        ...
        if isinstance(races, list) and races:
            ...
            if line_heal:
                # v7: レース単位で穴を埋める。
                #   会場ごと差し替えると、正常に取れていたレースまで
                #   上書きされてしまう。欠けているレースにだけ入れる。
                filled = heal_lines(engine, races, tdt, date_str)
                # S/B・race_kind・grade等も未付与なら足す
                _had_meta = races_already_has_meta(races)
                applied, total = apply_shb_to_races(races)
                _now_meta = races_already_has_meta(races)
                if filled == 0 and applied == 0 and _had_meta == _now_meta:
                    print("[line] 変わりませんでした (現状維持)")
                    cleanup_old(tdt)
                    return
                f = open(out_path, "w", encoding="utf-8")
                json.dump(races, f, ensure_ascii=False)
                f.close()
                print("[done] 補完保存: ライン " + str(filled)
                      + "R / S・B " + str(applied) + "R → " + out_path)
                cleanup_old(tdt)
                return
            ...
            if not weak:
                ...
                if (races_already_has_shb(races)
                        and races_already_has_kind(races)
                        and races_already_has_meta(races)):
                    print("[skip] 取得済み: " + str(len(races))
                          + "R (S/B・race_kind・grade等すべて付与済み)")
                    cleanup_old(tdt)
                    return
                ...
                if (applied > 0 or (_now_kind and not _had_kind)
                        or (_now_meta and not _had_meta)):
                    f = open(out_path, "w", encoding="utf-8")
                    json.dump(races, f, ensure_ascii=False)
                    f.close()
    ...
    # v7: 取り終えた直後に、ラインが欠けているレースを一度だけ埋める。
    #   ミッドナイトは掲載が遅いので、ここで埋まらないことも多い。
    #   その場合は後続の実行 (KEIRIN_LINE_HEAL=1) で埋める。
    try:
        heal_lines(engine, all_races, tdt, date_str)
    except Exception as e:
    ...
    _nl = count_lineless(all_races)
    if _nl:
        print("[line] まだライン未掲載: " + str(_nl) + "R"
              + " (後続の実行で埋める)")
    cleanup_old(tdt)
