...
def main():
    ...
    global REBUILD
    REBUILD = os.environ.get("BVDB_REBUILD", "").strip() == "1"
    if REBUILD:
        print("[mode] 作り直し (既存ファイルを無視して取り直します)")
    ...
    for d in targets:
        out_path = os.path.join(OUT_DIR, "BVDB_" + d + ".json.gz")
        old_path = os.path.join(OUT_DIR, "BVDB_" + d + ".json")
        if os.path.exists(out_path) and not REBUILD:
            print("[skip] %s (取得済み)" % d)
            continue
        if os.path.exists(old_path) and not REBUILD:
            # 非圧縮で作った古いものがあれば、それも取得済みとみなす
            print("[skip] %s (取得済み・非圧縮)" % d)
            continue
        ...
        import gzip
        f = gzip.open(out_path, "wt", encoding="utf-8")
...
def merge():
    ...
    全部を1つの BVDB.json にすると、16日ぶんで既に160MBになり、
    GitHubの100MB制限に弾かれた (しかも日別ファイルまで巻き添えで
    push されなくなる)。
    そこで月ごとに分け、さらに gzip で圧縮して置く。
    読むときは gzip.open で普通に開ける。
    """
    import glob
    import gzip
    files = sorted(glob.glob(os.path.join(OUT_DIR, "BVDB_????????.json"))
                   + glob.glob(os.path.join(OUT_DIR, "BVDB_????????.json.gz")))
    bymonth = {}
    for fp in files:
        base = os.path.basename(fp)
        ym = base[5:11]           # BVDB_YYYYMMDD.json(.gz) -> YYYYMM
        if not ym.isdigit():
            continue
        bymonth.setdefault(ym, []).append(fp)

    for ym in sorted(bymonth):
        allr = []
        for fp in sorted(bymonth[ym]):
            try:
                if fp.endswith(".gz"):
        ...
        out = os.path.join(OUT_DIR, "BVDB_" + ym + ".json.gz")
        body = {"month": ym,
                "generated": datetime.now(JST).strftime("%Y-%m-%d %H:%M:%S"),
        ...
        print("[merge] %s  %d日 %dレース -> %s (%.1fMB)"
              % (ym, len(bymonth[ym]), len(allr), out, mb))
        if mb > 90:
            print("::warning::%s が90MBを超えています (%.1fMB)" % (out, mb))
