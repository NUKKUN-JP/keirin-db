...
def venue_map_from_api():
    """会場名 -> 会場コード を API から作る。

    engine は pandas などを要求するため、環境によっては読み込めない。
    ところが必要なのは会場名とコードの対応だけで、
    それは race_detail の venues[] にそのまま入っている。
    どのレースを引いても同じ一覧が付いてくるので、
    実在するレースを1つ引ければ作れる。
    ...
    print("[find] 会場コードを API から作ります…")
    # 会場コードは2桁。実在する組み合わせに当たるまで順に試す。
        ...
        for i in range(1, 100):
            ...
            if m:
                try:
                    ...
                    pass
                print("[find] 会場コード: API から %d件" % len(m))
                return m
            time.sleep(SLEEP)
    return None
...
def main():
    ...
    idx, nrow = load_db(dbp)
    days = sorted(idx)

    # DBに出てくる会場名を全部集めてから対応表を作る。
    # 足りなければ API で補うので、会場が黙って欠けることがない。
    need = set()
    for d in idx:
        for p in idx[d]:
            need.add(p)
    n2c = venue_code_map(sorted(need))
    if not n2c:
        print("会場コードが得られません。")
        return 1
