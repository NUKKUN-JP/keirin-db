# -*- coding: utf-8 -*-
# ============================================================
# grade一括補填 v2 (収集→適用 一括) (2022-2024 の grade/race_kind/day_label/series_name)
# ============================================================
# 方針(性能重視):
...
def collect():
    ...
    try:
        ...
        g.close()
    print("[収集完了] 処理 " + str(processed) + " 件 / grade取得成功 "
          + str(ok) + " 件 → " + FILLS_PATH)
...
def main():
    collect_only = _env("GB_COLLECT_ONLY", "0") in ("1", "true", "yes")
    if APPLY_MODE:
        # 適用のみ
        apply_to_db()
        return
    ...
    collect()
    if collect_only:
        print("[収集のみモード] DB適用はスキップ(GB_COLLECT_ONLY=1)")
        return
    print("")
    print("=== 収集完了 → 続けてDBへ一括適用します ===")
    apply_to_db()
