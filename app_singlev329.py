...
def build_race_payload(race, venue_home_dir, bank_data, fast=False):
    ...
    while bi2 < len(bikes_sorted):
        ...
        _pid = pdata.get("player_key") or info.get("name") or ""
        # 長期離脱明け判定 (当日基準)
        try:
...
function markYosouHit(key, pay){
  ...
  if(!el) return;
  if(!won) return;
  var segs='<span class="seg hit">予想的中</span>';
  segs+='<span class="seg tri">'+tri+'</span>';
  if(rf.refund_3t) segs+='<span class="seg yen">'
    +Number(rf.refund_3t).toLocaleString()+'円</span>';
  segs+='<span class="seg">'+wonN+'点</span>';
  var rbot=el.querySelector(".rbot");
  if(rbot){
    rbot.innerHTML='<div class="rmarquee"><span class="track">'+segs+segs+'</span></div>';
  }
