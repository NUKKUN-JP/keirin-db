...
# 98MBの player_rsrank_finish_7car_FINAL.jsonl は除外 (月1・GitHub Releases経由で別管理)。
DICTS_SYNC_TARGETS = (
...
# 同期から除外する大容量ファイル (rawの25MB制限超・月1でReleases経由)
DICTS_SYNC_EXCLUDE = (
...
def sync_dicts_files():
    ...
    98MBの rsrank_7car は DICTS_SYNC_EXCLUDE で除外 (月1・Releases経由で別管理)。"""
    try:
        import requests
    except Exception:
        return (False, "requestsなし")
...
def load_races(date_str):
    ...
    # today_cacheが無い(過去日など)場合はDB本体から出走表を復元する。
    # 過去レースはDBに全項目(players/lap/line/weather/result)があるので
    # スクレイピング不要で一瞬。実績の月別walk-forward集計を高速化する。
    if not races:
        if RESULTS is not None:
            try:
                db_races = RESULTS.get_races_for_date(date_str)
            except Exception:
                db_races = []
            if db_races:
                races = db_races
...
def api_venues():
    ...
    # 第1R発走時刻順
    out_venues.sort(key=lambda x: x["first_post"] if x["first_post"] else "99:99")
...
<style>
  ...
  *{box-sizing:border-box;margin:0;padding:0;-webkit-tap-highlight-color:transparent}
  ...
  .oracle-meaning .mtxt{display:block;text-align:left}
  ...
  body.loading .race-grid > .rcell{animation:rcellWave 2.2s ease-in-out infinite}
  ...
  .rcell{background:linear-gradient(90deg, rgba(0,0,0,.30), rgba(20,24,15,.10));
    ...
    padding:0 12px 0 10px;transition:.15s;position:relative;height:46px;
    display:flex;flex-direction:row;align-items:center;gap:10px;cursor:pointer;
    box-shadow:inset 0 0 14px rgba(0,0,0,.40)}
  ...
  .rcell .rbot{flex:1;min-width:0;display:flex;align-items:center;
  ...
  .rcell.checking{animation:rblink 1s ease-in-out infinite}
  ...
  .bt-mv{color:#cfc4aa;letter-spacing:.06em;font-family:monospace}
  ...
  /* 予想の的中: 金のネオン */
  .hitbadge.yosou{color:#ffe9a8;border:1px solid #d8b24a;
  ...
  /* 酒場の的中: 赤のネオン */
  .hitbadge.tavern{color:#ffbfbf;border:1px solid #d84a4a;
    background:rgba(90,20,20,.35);font-weight:900;
    text-shadow:0 0 4px rgba(255,110,110,.95),0 0 9px rgba(255,60,60,.6);
    box-shadow:0 0 5px rgba(255,80,80,.5),inset 0 0 5px rgba(255,80,80,.18)}
  ...
  .fl-blab{font-size:10px;color:var(--txt-dim);min-width:104px;
    white-space:nowrap}
  .fl-buy{display:inline-block;font-family:monospace;font-size:12px;
    border:1px solid var(--line);border-radius:4px;padding:1px 4px;
    margin:1px 2px 1px 0;white-space:nowrap;color:var(--txt)}
  .fl-drop{display:inline-block;font-family:monospace;font-size:12px;
...
<div id="menuOverlay" class="menu-overlay" onclick="if(event.target===this)closeMenu()">
  <div class="menu-panel">
    ...
    <div class="menu-item" onclick="menuOpenBacktest()">結果分析</div>
    <div class="menu-item" onclick="menuOpenFixedLogic()">確定ロジック検証</div>
...
function onDateChange(){ updateDateDisp(); }
...
function menuOpenFixedLogic(){ closeMenu(); openFixedLogic(); }
...
function setStatus(html){
  ...
  if(on && document.body.classList.contains("oracle-loading")){
    html = '<div class="oracle-meaning"><span class="w2"><span class="gw">託宣</span>とは——。</span><span class="mtxt">神仏が人に乗り移り、また夢に現れて、その意志を告げ知らせること。</span></div>';
  }
...
function loadVenues(){
  ...
  _SUM_ROWS=null; _SUM_STATE="idle"; _SUM_PROG=""; _SUM_ABORT=true;
...
function fetchVenuesOnce(afterBuild){
  fetch("/api/venues?date="+DATE)
    ...
    .then(function(j){
      ...
      stopLoadingUI();
      setStatus("");
      fetchVenueMarks();   // 予想がある会場に色を付ける (予想計算は呼ばない)
      // v329: 背景集計はやめた。
...
function renderVenuesInner(){
  ...
  for(var i=0;i<VENUES.length;i++){
    var v=VENUES[i];
    var dly=" d"+((i%8)+1);
    html+='<div class="vchip anim'+dly+'" data-i="'+i+'" onclick="selectVenue('+i+')">'
        + '<span class="nm">'+v.name+'</span>'
        + v.first_post+' / '+v.race_count+'R</div>';
  }
  // v329: 会場ボタンの最後に「本日の集計」を同じ形で置く。
  //   押したときに初めて全会場を調べる (上位ボタンは何も読まない方針を守る)。
  html+='<div class="vchip anim d8 vsum" data-i="-1" onclick="showDaySummary()">'
      + '<span class="nm">本日の集計</span>予想と結果</div>';
  s.innerHTML=html;
}
...
//   まだ途中なら、進捗を出しつつ続きを待つ。
// ============================================================
var _SUM_ROWS=null, _SUM_STATE="idle", _SUM_PROG="", _SUM_ABORT=false;
...
function waitIdle(cb){
  if(_SUM_ABORT) return;
  if(!isBusy()){ setTimeout(cb, 250); return; }
  setTimeout(function(){ waitIdle(cb); }, 800);
}

function startDaySummaryBg(){
  if(_SUM_STATE==="running" || _SUM_STATE==="done") return;
  _SUM_STATE="running"; _SUM_ROWS=[]; _SUM_PROG=""; _SUM_ABORT=false;
  collectDaySummary(function(rows){
    _SUM_ROWS=rows; _SUM_STATE="done"; _SUM_PROG="";
    if(document.querySelector(".vchip.vsum.active")){ drawSummary(_SUM_ROWS); }
  }, function(msg){
    _SUM_PROG=msg;
    if(document.querySelector(".vchip.vsum.active")){
      var d=ensureDetailEl();
      if(d) d.innerHTML='<div class="skip-note"><span class="spin"></span>'+msg+'</div>';
    }
  });
}
...
function showDaySummary(){
  ...
  var last=document.querySelector(".vchip.vsum");
  if(last) last.classList.add("active");
  document.getElementById("raceGrid").innerHTML="";
  setOracle(null);
  var d=detachDetail();
  _BUSY_UNTIL=0;   // 集計を見たいので、こちらを優先に戻す
  if(_SUM_STATE==="done" && _SUM_ROWS){ drawSummary(_SUM_ROWS); return; }
  d.innerHTML='<div class="skip-note"><span class="spin"></span>'
    + (_SUM_PROG || "本日の予想を集めています…")+'</div>';
  if(_SUM_STATE!=="running") startDaySummaryBg();
}

// 全会場から「稼働条件に該当したレース」を集め、買い目と結果を突き合わせる。
//   done(rows) で完了、prog(msg) で進捗を知らせる。
function collectDaySummary(done, prog){
  var venues=[];
  for(var i=0;i<VENUES.length;i++) venues.push(VENUES[i].name);
  var picks=[], vi=0;

  function nextVenue(){
    if(_SUM_ABORT) return;
    if(vi>=venues.length){ evalPicks(); return; }
    if(isBusy()){ waitIdle(nextVenue); return; }
    var vn=venues[vi]; vi++;
    if(prog) prog(vn+" を調べています ("+vi+"/"+venues.length+")");
    var ck=DATE+"|"+vn;
    var qp = _QF_CACHE[ck]
      ? Promise.resolve({flags:_QF_CACHE[ck]})
      : fetch("/race/qflags?date="+DATE+"&venue="+encodeURIComponent(vn))
          .then(function(r){return r.json()}).catch(function(){return {}});
    var vp = fetch("/api/venue_flags?date="+DATE+"&venue="+encodeURIComponent(vn))
          .then(function(r){return r.json()}).catch(function(){return {}});
    Promise.all([qp,vp]).then(function(res){
      var q=(res[0]||{}).flags||{};
      var f=(res[1]||{}).flags||{};
      if(!_QF_CACHE[ck]) _QF_CACHE[ck]=q;
      for(var key in q){
        var pk=q[key].pick;
        if(!pk) continue;
        var ff=f[key]||{};
        if(ff.displayable!==true) continue;
        picks.push({key:key, venue:vn, pick:pk, flag:ff});
      }
      nextVenue();
    });
  }
  ...
  function evalPicks(){
    ...
    function step(){
      if(_SUM_ABORT) return;
      if(idx>=picks.length){ done(rows); return; }
      if(isBusy()){ waitIdle(step); return; }
      var P=picks[idx]; idx++;
      if(prog) prog("買い目を作っています ("+idx+"/"+picks.length+")");
      getPayload(P.key)
        .then(function(pay){
          try{ rows=rows.concat(buildSumRows(P,pay)); }catch(e){}
          step();
        })
        .catch(function(){ step(); });
    }
...
function buildSumRows(P, pay){
  // v329: 実際に買うのは pick 1つだけ。集計もそれに合わせる。
  var out=[];
  if(!pay || pay.status!=="ok") return out;
  if(typeof __btUnionRace!=="function") return out;
  var h=P.pick;
  if(!h) return out;
  var tri=(P.flag&&P.flag.trifecta)?P.flag.trifecta:"";
  var refund=(P.flag&&P.flag.refund_3t)?P.flag.refund_3t:0;
  var fin=(P.flag&&P.flag.finished===true);
  var u=null;
  try{ u=__btUnionRace(pay, tri||"0-0-0", refund, h.axes); }
  catch(e){ return out; }
  if(!u||!u.ok||!u.byN||!u.byN[h.n-1]) return out;
  var cb=u.byN[h.n-1].combos||[];
  var won=false;
  if(fin && tri){
    for(var k=0;k<cb.length;k++){ if(String(cb[k])===String(tri)){ won=true; break; } }
  }
  out.push({venue:P.venue, key:P.key, name:h.name, n:h.n,
            combos:cb, bet:cb.length*100, fin:fin, tri:tri,
            won:won, ret:(won?refund:0)});
  return out;
}
...
function drawSummary(rows){
  var d=ensureDetailEl();
  if(!rows.length){
    d.innerHTML='<div class="skip-note">買い目を作れたレースがありません。</div>';
    return;
  }
  var bet=0, ret=0, nfin=0, nwon=0;
  for(var i=0;i<rows.length;i++){
    bet+=rows[i].bet;
    ret+=rows[i].ret;
    if(rows[i].fin){ nfin++; if(rows[i].won) nwon++; }
  }
  var roi=bet? (100*ret/bet):0;
  var s='<div class="yos-card"><h4>本日の集計</h4>'
    + '<div class="sum-grid">'
    + '<div><span>買い目</span>'+rows.length+'件</div>'
    + '<div><span>確定</span>'+nfin+'件</div>'
    + '<div><span>的中</span>'+nwon+'件</div>'
    + '<div><span>投資</span>'+bet.toLocaleString()+'円</div>'
    + '<div><span>払戻</span>'+ret.toLocaleString()+'円</div>'
    + '<div><span>回収率</span>'+roi.toFixed(1)+'%</div>'
    + '<div><span>損益</span>'+(ret-bet>=0?"+":"")+(ret-bet).toLocaleString()+'円</div>'
    + '</div></div>';
  s+='<div class="yos-card"><h4>内訳</h4>';
  for(var j=0;j<rows.length;j++){
    var r=rows[j];
    var st= r.fin ? (r.won?'<span class="sum-hit">的中</span>'
                          :'<span class="sum-miss">不的中</span>')
                  : '<span class="sum-wait">発走前</span>';
    s+='<div class="sum-row">'
      + '<div class="sum-top">'+esc(r.venue)+' '+esc(String(r.key).split("_")[1])+'R'
      + ' <span class="sum-st">'+esc(r.name)+' '+r.n+'点</span> '+st+'</div>';
    for(var k=0;k<r.combos.length;k++){
      var on=(r.fin && r.tri && String(r.combos[k])===String(r.tri))?" won":"";
      s+='<span class="yos-combo'+on+'">'+esc(r.combos[k])+'</span>';
    }
    if(r.fin && r.tri){
      s+='<div class="sum-res">結果 '+esc(r.tri)
        + (r.ret?(' / '+Number(r.ret).toLocaleString()+'円'):'')+'</div>';
    }
    s+='</div>';
  }
  s+='</div>';
...
function renderRaceGrid(i){
  ...
  for(var j=0;j<v.races.length;j++){
    ...
    html+='<div class="'+cls+'" id="rc_'+cssId(r.key)+'" data-key="'+r.key+'" onclick="tapRace(\''+r.key+'\',this)">'
        ...
        + '<div class="rtop"></div>'
        + '<div class="rpred" id="rp_'+cssId(r.key)+'"><span class="wait">判定中</span></div>'
        + '<div class="rbot"></div>'
        + '</div>';
  }
  g.innerHTML=html;
...
function fetchVFlags(venueName){
  ...
  if(_VF_CACHE[ck]){ applyVFlags(_VF_CACHE[ck]); return; }
...
function applyVFlags(fl){
  for(var key in fl){
    ...
    // 予想 (買う候補があり、かつ計算できるレースだけ)
    var box=document.getElementById("rp_"+cssId(key));
    if(box){
      if(f.pick && f.displayable===true){
        el.classList.add("yosou");
        box.innerHTML='<span class="yes">予想 '+f.pick.n+'点</span>';
      }else{
        el.classList.remove("yosou");
        box.innerHTML='<span class="no">見送り</span>';
      }
    }
    // 結果
    var rbot=el.querySelector(".rbot");
    if(rbot && f.finished===true){
      rbot.innerHTML='<span class="rt finmk">終了</span>';
    }
  }
  checkYosouHits();
}
...
function paintYosou(){
  if(!_QF_DONE) return;
  for(var key in _YOSOU_HITS){
    var el=document.getElementById("rc_"+cssId(key));
    if(!el) continue;
    var box=document.getElementById("rp_"+cssId(key));
    if(!box) continue;
    var f=_YOSOU_HITS[key];
    var pk=(f && f.pick) ? f.pick : null;
    var okCalc=(_DISPLAYABLE[key]===true);
    // v329: 買うのは1つだけ。点数の少ないもの(同点なら前月回収率が高い方)。
    if(pk && okCalc){
      el.classList.add("yosou");
      box.innerHTML='<span class="yes">予想 '+pk.n+'点</span>';
    }else{
      el.classList.remove("yosou");
      box.innerHTML='<span class="no">見送り</span>';
    }
...
function checkYosouHits(){
  if(_HIT_TIMER){ clearTimeout(_HIT_TIMER); }
  _HIT_TIMER=setTimeout(function(){
    if(isBusy()){ checkYosouHits(); return; }
    checkYosouHitsNow();
  }, 2500);
}

function checkYosouHitsNow(){
  var queue=[];
  for(var key in _YOSOU_HITS){
    if(_YHIT_CHECKED[key]) continue;
    var f=_YOSOU_HITS[key];
    if(!f || !f.pick) continue;
    if(_DISPLAYABLE[key]!==true) continue;
    var rf=_RESULTMAP[key];
    if(!rf || rf.finished!==true || !rf.trifecta) continue;
    queue.push(key);
  }
  ...
  function step(){
    ...
    var key=queue[idx]; idx++;
    _YHIT_CHECKED[key]=true;
    getPayload(key)
      .then(function(pay){
        try{ markYosouHit(key, pay); }catch(e){}
        step();
      })
      .catch(function(){ step(); });
  }
...
function markYosouHit(key, pay){
  if(!pay || pay.status!=="ok") return;
  if(typeof __btUnionRace!=="function") return;
  var rf=_RESULTMAP[key]||{};
  var tri=rf.trifecta||"";
  if(!tri) return;
  var f=_YOSOU_HITS[key]||{};
  var pk=f.pick;
  if(!pk) return;
  var won=false, wonN=pk.n, u=null;
  try{ u=__btUnionRace(pay, tri, rf.refund_3t||0, pk.axes); }catch(e){ return; }
  if(!u || !u.ok || !u.byN || !u.byN[pk.n-1]) return;
  var cb=u.byN[pk.n-1].combos||[];
  for(var k=0;k<cb.length;k++){
    if(String(cb[k])===String(tri)){ won=true; break; }
  }
  var el=document.getElementById("rc_"+cssId(key));
  if(!el) return;
  if(!won) return;
  // v330: 流れるマーキーをやめ、角丸のバッジで出す。
  //   予想の的中は金、酒場の的中は赤。どちらの的中かが一目で分かる。
  var h = '<span class="hitbadge yosou">予想</span>';
  h += '<span class="hitmk">的中</span>';
  h += '<span class="hittri">' + tri + '</span>';
  if (rf.refund_3t) {
    h += '<span class="hityen">'
      + Number(rf.refund_3t).toLocaleString() + '円</span>';
  }
  var rbot=el.querySelector(".rbot");
  if(rbot){ rbot.innerHTML = '<div class="hitline">' + h + '</div>'; }
    ...
    .then(function(j){
      ...
      for(var k=0;k<cells.length;k++){
        ...
        if(f.finished===true && false){
          ...
          // 終了済み(非的中)は「終了」だけ
          var rbot2=el.querySelector(".rbot");
          if(rbot2) rbot2.innerHTML='<span class="rt finmk">終了</span>';
        }
      }
      finishProgress();
      setStatus("");
...
function tapRace(key,el){
  ...
  setLoading(true);
  getPayload(key)
    .then(function(j){ finishProgress(); renderDetail(j); setLoading(false); })
    .catch(function(e){ stopFakeProgress(); setLogoThinking(false); d.innerHTML='<div class="skip-note">計算エラー: '+e+'</div>'; setLoading(false); });
}

// 読み込み中ブロック: プログレスバー + 進捗% (ロゴは右上が考え中になる)
function loadingBlock(){
  return '<div class="loadwrap">'
    + '<div class="loadbar"><div id="loadbarfill" class="loadbar-fill" style="width:0%"></div></div>'
    + '<div id="loadpct" class="loadpct">0%</div>'
    + '</div>';
}
...
function startFakeProgress(){
  ...
  setLogoThinking(true); // 右上ロゴを考え中に
  var fill=document.getElementById("loadbarfill");
  var lab=document.getElementById("loadpct");
  // transitionを一旦切って 0% を即時確定 (前回値が一瞬見えるのを防ぐ)
  if(fill){
    fill.style.transition="none";
    fill.style.width="0%";
    void fill.offsetWidth; // リフロー強制で0%を確定
    fill.style.transition="";
  }
  if(lab) lab.textContent="0%";
  var pct=0;
  _fakeTimer=setInterval(function(){
    // 95%で頭打ち。実際の完了(finishProgress)まで100%にはしない
    var inc = pct<60 ? 5 : (pct<85 ? 1.5 : 0.4);
    pct=Math.min(95, pct+inc);
    var f=document.getElementById("loadbarfill");
    var l=document.getElementById("loadpct");
    if(f) f.style.width=pct+"%";
    if(l) l.textContent=Math.floor(pct)+"%";
    if(pct>=95){ clearInterval(_fakeTimer); _fakeTimer=null; }
  }, 110);
}
// 擬似進捗のタイマーだけ止める (バー・ロゴは変更しない: エラー時等の保険)
function stopFakeProgress(){
  if(_fakeTimer){ clearInterval(_fakeTimer); _fakeTimer=null; }
}
// 実際の処理完了時に呼ぶ: 100%にして考え中を自然解除
function finishProgress(){
  ...
  setLogoThinking(false); // 右上ロゴを通常動作へ自然に復帰
}
...
function renderYosou(pane){
  ...
  loadConditions(function(C){ drawYosou(pane, C); });
  if(!_COND && !_COND_TRIED){
    pane.innerHTML='<div class="skip-note"><span class="spin"></span>条件を読んでいます…</div>';
  }
}

function drawYosou(pane, C){
  if(!C || !C.strategies || !C.strategies.length){
    pane.innerHTML='<div class="skip-note">conditions.json が読めていません。<br>'
      + 'app と同じ場所か takusen/data に置いてください。</div>';
    return;
  }
  if(typeof __btUnionRace!=="function"){
    pane.innerHTML='<div class="skip-note">oracle_core.js が読めていません。<br>'
      + 'takusen/data か Download に置いてください。</div>';
    return;
  }
  // v329: Q・分戦・該当判定はサーバ(/race/qflags)の結果をそのまま使う。
  //   ブラウザ側で raw_score から計算し直すと、条件を選別したときの式
  //   (競走得点 - 平均着順*5 + グレード補正) と食い違い、
  //   Rボタンは「予想あり」なのにタブは「見送り」という矛盾が起きる。
  var ven=((_RACE||{}).header||{}).venue||"";
  var qf=_YOSOU_HITS[_CUR_KEY]||{};
  var q=qf.q||"", lc=qf.line||"";
  if(!q || !lc){
    pane.innerHTML='<div class="skip-note">判定を読んでいます…</div>';
    return;
  }
  var s='<div class="yos-head">'
    + '<span class="yos-tag on">'+q+'</span>'
    + '<span class="yos-tag on">'+lc+'</span>'
    + '<span class="yos-tag">'+esc(ven)+'</span></div>';

  // v329: 該当が複数あっても、買うのは点数の少ない1つだけ。
  //   同点なら前月回収率が高い方。
  var pick=qf.pick||null;
  var nBuy=0;
  if(pick){
    var u=null;
    try{ u=__btUnionRace(_RACE,"0-0-0",0,pick.axes); }catch(e){ u=null; }
    if(u && u.ok && u.byN && u.byN[pick.n-1]){
      var cb=u.byN[pick.n-1].combos||[];
      var tags="";
      for(var k=0;k<cb.length;k++){ tags+='<span class="yos-combo">'+esc(cb[k])+'</span>'; }
      s+='<div class="yos-card"><h4>'+esc(pick.name)+'</h4>'
        + '<div class="yos-row"><div class="yos-sub">'+pick.n+'点'
        + '<span class="yos-prev">前月回収 '+pick.roi_prev+'%</span></div>'
        + tags+'</div></div>';
      nBuy=1;
    }
  }
  if(!nBuy){
    s+='<div class="yos-none">該当する条件なし — <b>見送り</b></div>';
  }
  s+='<div class="yos-note">条件 '+esc(C.updated||"")+' 時点。該当が複数でも1つだけ買う。<br>'
    + '<span class="yos-warn">該当レース数が少ない条件が多い。'
    + '数字が良く見えても母数を確認すること。</span></div>';
  pane.innerHTML=s;
}
...
function menuOpenBacktest(){closeMenu();openBacktest();}
