...
<style>
  ...
  *{box-sizing:border-box;margin:0;padding:0;-webkit-tap-highlight-color:transparent}
  ...
  /* 語り (タイプライター) */
  .maria{margin-top:14px;text-align:left}
...
function loadVenues(){
  ...
  var _lp=document.getElementById("loadphase");
  if(!_lp){
    var g0=document.getElementById("raceGrid");
    if(g0){ g0.innerHTML='<div id="loadphase" class="loadphase"></div>';
      _lp=document.getElementById("loadphase"); drawProgress(); }
  }
  typeMaria(_lp);
...
function typeMaria(el){
  ...
  function tick(){
    ...
    var txt = para[pi];
    if(ci < txt.length){
      cur.textContent = txt.substring(0, ci+1);
      ci = ci + 1;
      _typeTimer = setTimeout(tick, 55);
    }else{
      pi = pi + 1; cur = null;
      _typeTimer = setTimeout(tick, 620);   // 段落の間
    }
  }
  tick();
}
