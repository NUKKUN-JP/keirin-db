...
<style>
  ...
  *{box-sizing:border-box;margin:0;padding:0;-webkit-tap-highlight-color:transparent}
  ...
  .yos-form .fhit{color:#7fe08a;font-weight:800;
    text-shadow:0 0 7px rgba(110,235,130,.85)}
  .yos-form .fh{color:#cfc4aa}
  .yos-form .fmark{margin-left:9px;font-size:10px;font-weight:800;
    letter-spacing:.1em;color:#7fe08a;border:1px solid #4f9a5c;
    border-radius:5px;padding:1px 7px;background:rgba(20,60,30,.4);
    text-shadow:0 0 5px rgba(110,235,130,.8);
    box-shadow:0 0 6px rgba(90,220,110,.35)}
  ...
  .loadphase{margin:14px auto;max-width:340px;padding:0 8px}
    ...
    while (i < segs.length) {
      ...
      if (tp && pp.length === 3) {
        ...
        while (k < th.length) {
          var c = th.charAt(k);
          k = k + 1;
          var on = (headOk && c === tp[2]);
          body += '<span class="' + (on ? "fhit" : "") + '">' + esc(c)
                + '</span>';
        }
        if (headOk && th.indexOf(tp[2]) >= 0) {
          body += '<span class="fmark">的中</span>';
        }
      } else {
        body = esc(seg);
      }
