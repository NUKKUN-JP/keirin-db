...
<style>
  ...
  *{box-sizing:border-box;margin:0;padding:0;-webkit-tap-highlight-color:transparent}
  ...
  .rcell{background:linear-gradient(90deg, rgba(0,0,0,.30), rgba(20,24,15,.10));
    ...
    padding:6px 12px 6px 10px;transition:.15s;position:relative;height:62px;
  ...
  /* 狙いレースの枠 */
  .rcell.target{border-left:2px solid var(--gold-strong);
    background:linear-gradient(90deg, rgba(255,190,100,.13), rgba(60,40,15,.05))}
  ...
  loadPicks(function(){
    ...
    for(var i=0;i<cells.length;i++){
      ...
      if(!p){
        ...
        box.innerHTML="";
        if(bar) bar.innerHTML='<i class="lv0" style="width:0%"></i>';
      ...
      if(p.target){
        ...
        box.innerHTML='<span class="tgtmark">狙い</span>';
      }else{
        el.classList.remove("target");
        box.innerHTML="";
      }
...
function markYosouHit(key){
  ...
  if(p.target && inList(p.combos)){
    kind="target"; name="狙い予想";
  }else{
    var st=p.steps||[];
    for(var m=0;m<st.length;m++){
      if(inList(st[m].combos)){
        kind="step";
        name=st[m].name+"（約"+st[m].approx+"点）";
        break;   // 先頭ほど絞っているので最初に当たったものを使う
      }
    }
  }
  if(!kind) return;

  var h='<span class="hitbadge '+kind+'">'+name+'</span>';
  h+='<span class="hitmk">的中</span>';
  h+='<span class="hittri">'+tri+'</span>';
  if(yen) h+='<span class="hityen">'+Number(yen).toLocaleString()+'円</span>';
  var rbot=el.querySelector(".rbot");
...
function selectStep(n){
  ...
  var pane = document.getElementById("pane_yosou");
  if(!pane){
    var tb = document.querySelector(".tabpane.active");
    if(tb) pane = tb;
  }
...
function formHtml(formStr, list){
  if (formStr) {
    ...
    var i = 0;
    while (i < segs.length) {
      s += '<div class="yos-form">' + esc(segs[i]) + '</div>';
      i = i + 1;
    }
    return s;
  }
