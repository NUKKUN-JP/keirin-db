...
<style>
  ...
  *{box-sizing:border-box;margin:0;padding:0;-webkit-tap-highlight-color:transparent}
  ...
  .ar-top{display:flex;justify-content:space-between;align-items:baseline;
    margin-bottom:7px}
  ...
  .yos-form{font-family:monospace;font-size:15px;color:#ffe6a8;
...
function paintYosou(){
  ...
  loadPicks(function(){
    ...
    for(var i=0;i<cells.length;i++){
      ...
      if(p && p.points){
        el.classList.add("yosou");
        box.innerHTML='<span class="yes">予想 '+p.points+'点</span>';
      }else{
        el.classList.remove("yosou");
        box.innerHTML='<span class="no">見送り</span>';
      }
...
function arareBar(star, predPay){
  ...
  s += '<span class="ar-val ' + lv + '">' + star + ' / 5</span></div>';
...
function drawYosou(pane){
  ...
  var i = 0;
  while (i < fr.length) {
    s += '<span class="yos-tag on">' + esc(String(fr[i].label))
  ...
  // 買い目 (フォーメーション優先)
  s += '<div class="yos-card"><h4>買い目　計' + p.points + '点</h4>';
  if (p.formation_ok && p.formation) {
    var segs = String(p.formation).split(" / ");
    var j2 = 0;
    while (j2 < segs.length) {
      s += '<div class="yos-form">' + esc(segs[j2]) + '</div>';
      j2 = j2 + 1;
    }
  } else {
    var cb = p.combos || [];
    ...
    var k = 0;
    while (k < cb.length) {
      tags += '<span class="yos-combo">' + esc(cb[k].t || cb[k]) + '</span>';
      k = k + 1;
    }
    s += '<div class="yos-row">' + tags + '</div>';
  }
  s += '</div>';
  ...
  if (fr.length > 1) {
    ...
    while (i < fr.length) {
      ...
      var m = 0;
      while (m < c2.length) {
        t2 += '<span class="yos-combo sm">' + esc(c2[m]) + '</span>';
        m = m + 1;
      }
      s += '<div class="yos-row"><div class="yos-sub">'
         + esc(String(f.label)) + '　' + String(f.points) + '点設定'
         + '</div>' + t2 + '</div>';
    }
    s += '</div>';
  }
  ...
  s += '<div class="yos-note">'
     + (_PICKS_META.conditions_updated
        ? '条件 ' + esc(_PICKS_META.conditions_updated) + ' 時点　' : '')
     + (_PICKS_META.generated
        ? '生成 ' + esc(_PICKS_META.generated) : '')
     + '<br>重なった買い目は1点にまとめている。</div>';
  pane.innerHTML = s;
}
