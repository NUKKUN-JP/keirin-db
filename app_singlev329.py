...
<style>
  ...
  *{box-sizing:border-box;margin:0;padding:0;-webkit-tap-highlight-color:transparent}
  ...
  /* v330: 会場ボタンのバンク要目 */
  .vchip .nm{display:block;margin:0 0 5px;font-size:14px;letter-spacing:.1em}
  .vchip .bk{display:block;font-size:10px;line-height:1.75;letter-spacing:.04em}
  .vchip .bk-r{display:flex;gap:5px;align-items:baseline}
  .vchip .bk-l{font-style:normal;color:#6f6552;min-width:34px;flex:0 0 auto}
  .vchip .bk-v{color:#cfc4aa;font-weight:700;font-family:monospace}
  .vchip .bk-v.hi{color:#e8c88a}
  .vchip .bk-v.lo{color:#9db6c4}
  .vchip .bk-a{font-style:normal;color:#5d5646;font-size:9px}
  /* 狙いレースのある会場 */
  .vchip.has-target{border-left:3px solid #ffab44;
    background:linear-gradient(90deg, rgba(255,171,68,.13), rgba(20,24,15,.10))}
  .vchip.has-target .nm{color:#ffab44}
  ...
  .rcell .gmark{display:inline-block;padding:1px 6px;border-radius:2px;
    font-size:10px;font-weight:700;letter-spacing:.08em;line-height:1.5;
    font-family:'Helvetica Neue',Arial,sans-serif}
  .rcell .g-g1{background:#c33a3a;color:#fff}
  .rcell .g-g2{background:#7a4bb5;color:#fff}
  .rcell .g-g3{background:#2f8a55;color:#fff}
  .rcell .g-f1{background:#2f6ba8;color:#fff}
  .rcell .g-f2{background:#5c5c5c;color:#e6e6e6}
  .rcell .g-other{background:#3a3527;color:#cfc4aa}
  ...
  .hittrack{display:inline-block;padding-left:8px;
    animation:hitFlow 13s linear infinite}
  ...
  /* 語り (タイプライター) */
  .maria{margin-top:16px;text-align:left;max-width:420px;
    margin-left:auto;margin-right:auto}
  .maria .mline{margin:0 0 9px;font-size:13px;line-height:1.95;
    color:#d9cbab;letter-spacing:.06em;text-shadow:0 0 9px rgba(255,220,160,.16)}
  .maria .mline:first-child{color:#ffe6a8;font-size:14px;letter-spacing:.12em}
  ...
  if(on && document.body.classList.contains("oracle-loading")){
    ...
    if(!_st.querySelector(".oracle-meaning")){
      _st.innerHTML = '<div class="oracle-meaning">'
        + '<span class="w2"></span><span class="mtxt"></span>'
...
function renderVenuesInner(){
  ...
  for(var i=0;i<VENUES.length;i++){
    ...
    var b=v.bank||{};
    var bk='<div class="bk">'
      + _bkRow("周長", b.circ,     BANK_AVG.circ,     "m")
  ...
  html+='<div class="vchip anim d8 vsum" data-i="-1" onclick="showDaySummary()">'
      + '<span class="nm">本日の集計</span>'
      + '<div class="bk"><span class="bk-r">予想と結果</span></div></div>';
  s.innerHTML=html;
}
...
function markTargetVenues(){
  loadPicks(function(){
    ...
    for(var key in _PICKS){
      var p=_PICKS[key];
      if(p && p.target){ has[String(key).split("_")[0]]=1; }
    }
    var chips=document.querySelectorAll(".vchip");
    for(var k=0;k<chips.length;k++){
      var idx=parseInt(chips[k].dataset.i);
...
function typeOracle(root){
  if(!root) return;
  stopMaria();
  var head = root.querySelector(".w2");
  var mtxt = root.querySelector(".mtxt");
  var box  = root.querySelector(".maria");
  if(!head || !mtxt || !box) return;

  var para = MARIA[Math.floor(Math.random()*MARIA.length)];
  var jobs = [];
  jobs.push({t:"託宣とは——。", el:head, head:true});
  jobs.push({t:"神仏が人に乗り移り、また夢に現れて、その意志を告げ知らせること。", el:mtxt});
  for(var i=0;i<para.length;i++) jobs.push({t:para[i], box:box});

  var ji=0, ci=0, cur=null;
  function tick(){
    if(ji >= jobs.length){ _typeTimer=null; return; }
    var jb = jobs[ji];
    if(cur === null){
      if(jb.box){
        cur=document.createElement("p"); cur.className="mline"; jb.box.appendChild(cur);
      }else{
        cur=jb.el; cur.textContent="";
      }
      ci=0;
    }
    if(ci < jb.t.length){
      ci = ci + 1;
      var sub = jb.t.substring(0, ci);
      if(jb.head){
        // 冒頭2文字「託宣」だけ金色にする
        if(ci <= 2) cur.innerHTML = '<span class="gw">'+esc(sub)+'</span>';
        else        cur.innerHTML = '<span class="gw">託宣</span>'+esc(sub.substring(2));
      }else{
        cur.textContent = sub;
      }
      ...
      ji = ji + 1; cur = null;
      _typeTimer = setTimeout(tick, jb.head ? 520 : 640);   // 段落の間
    }
  }
  tick();
}
