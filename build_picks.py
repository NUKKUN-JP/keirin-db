...
【なぜ2段か】
検証A/B/Cの和集合ロジックは oracle_core.js にあり、
Python側には無い。実測で確認した対応は
...
切り出すより import する方が安全。アプリ側を直せば予想も追随する。
app.run() は __name__ ガードの中なので、import してもサーバは起動しない。

【★の計算】
arare_star_12m.json の重みとテーブルから予測配当を出し、
cuts と比べて★1〜5を決める。学習は202501-202512。
...
def main():
    ...
    print("")
    print("条件に合ったレース " + str(len(out)) + "R")
    print("  7車立てでない等 " + str(n_skip) + "R")
    print("  条件に合わない   " + str(n_nocond) + "R")
