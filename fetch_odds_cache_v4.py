# -*- coding: utf-8 -*-
# ============================================================
# gamboo 2車単オッズ 収集・キャッシュ化 v4 (実表対応パーサ・2shatan)
# ============================================================
...
def parse_gamboo_exacta_odds(html):
    """gamboo 2車単オッズ表を {"first-second": float} で返す。
    実表構造: ヘッダ行の列=1着車番、各データ行頭=2着車番、セル=オッズ。
    1着=列番号, 2着=行番号(順序あり)。対角(同一車)は空。"""
    out = {}
    if not html:
        return out
    ...
    for tbl in tables:
        ...
        if len(rows) < 3:
            continue
        # ヘッダ行: 列見出しの1着車番(1..7)を左から順に集める
        header_cars = []
        hcells = re.findall(r"<t[hd][^>]*>(.*?)</t[hd]>", rows[0], re.S)
        for c in hcells:
            t = re.sub(r"<[^>]+>", "", c).strip()
            if t.isdigit() and 1 <= int(t) <= 9:
                header_cars.append(int(t))
        if len(header_cars) < 3:
            continue
        n_head = len(header_cars)
        found_here = {}
        ri = 1
        while ri < len(rows):
            cells = re.findall(r"<t[hd][^>]*>(.*?)</t[hd]>", rows[ri], re.S)
            ri = ri + 1
            if not cells:
                continue
            texts = []
            for c in cells:
                texts.append(re.sub(r"<[^>]+>", "", c).strip())
            if not texts[0].isdigit():
                continue
            second = int(texts[0])
            # 行頭(2着)の次から、header_cars の順に1着を対応づけ。
            # 末尾に行番号の再掲(2着)が来る表もあるので n_head 個だけ読む。
            take = 0
            ci = 1
            while ci < len(texts) and take < n_head:
                first = header_cars[take]
                val = texts[ci]
                ci = ci + 1
                take = take + 1
                if first == second:
                ...
                if fv is not None and fv > 0:
                    found_here[str(first) + "-" + str(second)] = fv
        # このtableで十分な数のペアが取れたら採用
        if len(found_here) >= 6:
            return found_here
        if found_here and not out:
            out = found_here
    return out
...
def main():
    print("=== gamboo 2車単オッズ収集 v4  " + START_DATE + "〜" + END_DATE + " ===")
    if _rq is None:
