...
環境変数:
  ...
  KEIRIN_DATES        複数日 (カンマ/空白区切り 例 '20260614,20260616')
  KEIRIN_DATE_FROM    範囲取得の開始日 YYYYMMDD (KEIRIN_DATE_TO と併用)
  KEIRIN_DATE_TO      範囲取得の終了日 YYYYMMDD (両端含む)
...
def _build_date_list():
    ...
    優先順位:
      1. KEIRIN_DATES   カンマ/空白区切りの複数日 (例 '20260614,20260616')
      2. KEIRIN_DATE_FROM + KEIRIN_DATE_TO  連続範囲 (両端含む)
      3. KEIRIN_DATE    単一日 (従来互換)
      4. いずれも空 → 今日 (Asia/Tokyo)
    重複は除き、昇順で返す。不正な日付は警告して除外。"""
    def _norm(s):
        s = (s or "").strip()
        if not s:
            return None
    ...
    dates_raw = os.environ.get("KEIRIN_DATES", "").strip()
    if dates_raw:
        parts = dates_raw.replace(",", " ").split()
        pi = 0
        while pi < len(parts):
            d = _norm(parts[pi])
    ...
    if not out:
        d_from = _norm(os.environ.get("KEIRIN_DATE_FROM", ""))
        d_to = _norm(os.environ.get("KEIRIN_DATE_TO", ""))
        if d_from and d_to:
    ...
    if not out:
        d = _norm(os.environ.get("KEIRIN_DATE", ""))
        if d:
            out.append(d)
