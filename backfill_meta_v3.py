# -*- coding: utf-8 -*-
# ============================================================
# メタ一括補填 v3.1 (race_kind抽出を修正: 発走予定の後ろ・級班+段階)
# ============================================================
# 方針(性能重視):
...
def extract_race_meta(html):
    ...
    # 【v3.1修正】race_kind は「発走予定」の前ではなく後ろにある。
    #   実例: "... 発走予定 10:45 投票締切 10:40 チャレンジ予選"
    #   従来は前を探して失敗し、"前期２班""本命山根"等のゴミも拾っていた。
    # 級班(必須) + 段階(必須) のパターンで直接拾う。位置に依存しない。
    # まず .race_title_header 内を優先的に探す(最も確実)。
    KIND_RE = re.compile(
        r'(Ｓ級|Ａ級|チャレンジ|ガールズ|Ｌ級|Ｆ級)'
        r'([^\s、。]{0,8}?)'
        r'(予選|準決勝|決勝|特選|選抜|一般|優秀)')

    def _norm_kind(cls, mid, stage):
        # 級班を正規化。チャレンジ/ガールズはそのまま級班扱い。
        base = cls
        # 中間の飾り(初日/特/一/二/1/2 等)は段階側に寄せず級班+段階のみ返す
        return base + stage
    ...
    for el in soup.find_all(True):
        ...
        if not cl:
            continue
        if "race_title_header" not in " ".join(cl):
            continue
        htxt = el.get_text(" ", strip=True)
        mm = KIND_RE.search(htxt)
        if mm:
            kind = mm.group(1) + mm.group(3)
            break
    # 見つからなければ本文全体から
    if not kind:
        mm = KIND_RE.search(text_norm)
        if mm:
            kind = mm.group(1) + mm.group(3)
    if kind:
        out["race_kind"] = kind
